    ...
    if RZZ_twirl:
        for j in range(0, num_spins - 1):
            for k in range(j+1,num_spins):
                ...
                ## Flip theta as Paulis anti-commute, else remains same
...
def run_qc_quantum_step(model: IsingEnergyFunction,gamma:float, time:float ,SPAM_twirl:bool = True, RZZ_twirl:bool = False
):
